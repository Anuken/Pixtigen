...
import io.anuke.pixtigen.generator.TreeGenerator.Pixel;
import io.anuke.pixtigen.util.*;
import io.anuke.pixtigen.util.Value.CrystalValue;
import io.anuke.pixtigen.util.Value.FloatValue;
...
public enum FilterType{
	...
	public final void initValues(ValueMap values){
		this.values = values;
		init();
	}

	protected void value(String name, Value<?> value){
		values.add(name, value.clone());
	}
	...
	private FilterType(boolean applied){
		this();
	...
	public final float apply(Filter filter, TreeGenerator tree, Pixmap pixmap, Pixel[][] materials, Pixel pixel, int x, int y, int cy, int width, int height){
		this.pixel = pixel;
		this.tree = tree;
		this.pixmap = pixmap;
